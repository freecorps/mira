...
**From the dashboard.** Settings → Connections → **Add endpoint**, pick
**OpenCode Go**, paste the key, **Test connection**, save. The URL and the
quirks come from the preset, and the key is stored in Mira's database — no
file to edit and no process to restart. See [endpoints.md](endpoints.md).

**From the config file**, if you would rather:
...
the preset in `providers.json`, and the config then reads as the plain
...
for indexing, and an endpoint added from this preset starts on the first of
those. Both sit in the $60/month allowance tier; the models with a $15 tier
(Kimi K3, GLM-5.3, DeepSeek V4 Pro, Qwen3.8 Max, GPT-5.6 Luna, Grok 4.6) run
out sooner on a busy repository. Prices in the registry are the base
...
Kimi K2.7 Code is fixed at its own sampling temperature and refuses any
other. Mira notices the refusal, drops the field and retries, then leaves it
out of that model's later calls — reviews run at the model's default rather
than failing over a sampling knob.
...
The OpenCode Go card under **Settings → Connections** shows three meters —
**5-hour**, **weekly** and **monthly** — with how much of each is spent and
when it resets, read from `GET /zen/go/v1/usage` (the same document the Zen
console's Go page draws from). The snapshot is asked for when the page opens
and the stored one is more than five minutes old; **Refresh usage** asks
right away and reports the provider's own words when it refuses ("Invalid API
key.", "OpenCode Go subscription required.").
...
The snapshot is kept in the settings table under a digest of the key, so a
rotated key starts from an empty meter and one endpoint's refusal says
...
* **Zen is not Go.** The pay-per-token catalogue at `https://opencode.ai/zen/v1`
  (Claude, GPT, Gemini) is a separate endpoint on the same key, and its own
  preset — add it as a second endpoint if you want both. It has no usage
  meters, and its ids are its own.
