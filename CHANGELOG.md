...
- **Local review (`mira local review`).** The review Mira runs on a pull request, run against a change that is not one yet: the working tree, the index (`--staged`), or a commit range (`--range main...HEAD`, where the three-dot form compares against the merge base exactly as a pull request does). It is the same review, and that is the whole claim — the engine, the configuration loader, the retrieval of indexed context and learned rules, the pre-merge check framework and the rendered output are all imported from where the server gets them, with the JSON finding shape now emitted by one serialiser that `mira review` shares, so the two cannot drift. What is new is only what is genuinely local.
  ...
  **One destination, and no flag to change it.** Before a byte of the diff is read, the endpoint, credential, API protocol and model *vendor* the run is about to use are compared with the ones the repository is configured for, on all three tiers that see repository content — a guard that only looked at the review tier would let `indexing_model` carry the same source to a second vendor. Moving between one vendor's small and large models stays allowed, because that is a cost decision and not a different recipient. Two supporting rules make it mean something: `.mira.yaml` is resolved from the repository root rather than by walking up from the current directory, so reviewing a sibling checkout applies that checkout's rules and not this one's; and `--config` layers *underneath* it as deployment defaults, exactly as `mira serve --config` does, rather than replacing it.
