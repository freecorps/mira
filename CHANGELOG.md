...
- **Read-only MCP server (`mira mcp serve`).** Mira's recorded knowledge, offered to an agent over the MCP stdio transport: review findings and the feedback against them, the approved learned rules and their rationale, the evaluations behind those rules, and the indexed summary of a file with its symbols and dependents. An agent reviewing a change can already read the code; what it could not see is which findings were raised before and whether they held up.
  ...
  **Bounded and audited.** Every listing is a page; `limit` is capped by the server rather than refused, cursors are opaque and carry a fingerprint of the filters they came from so replaying one against a different query is an error instead of a walk from a meaningless position, and a response over the ceiling is reduced by dropping rows before it is reduced by shortening fields — with the cursor still pointing at the row after the last one actually returned. Every call, answered or refused, is written to the application database and to stderr, with arguments redacted and the returned rows never copied; refusals are the rows that matter, since an agent repeatedly asking for a repository it was not granted is the only attack shape this surface has. A failed audit write degrades the trail rather than the read. `mira mcp tools` prints the inventory without starting a session and `mira mcp audit` reads the trail back. The transport is stdio only — no network listener, no port to authenticate — and no new dependency: the protocol is a JSON-RPC framing loop and a method table. See [docs/mcp.md](docs/mcp.md).
